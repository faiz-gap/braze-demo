...
const PromoCard = ({ card, isBrazeSourced, onCtaClick }) => {
  ...
  const openInNewTab = isBrazeSourced ? card.openUrlInNewTab : false; // Assume default opens in same tab

  const handleCtaClick = (e) => {
    if (ctaUrl === '#') {
      e.preventDefault(); // Prevent navigation if URL is just '#'
    }
    if (onCtaClick) {
      onCtaClick(); // This should handle Braze click logging
    }
    // Navigation will be handled by the <a> tag's href
  };

  return (
    <div
      id={id}
      className="bg-white rounded-xl shadow-card overflow-hidden flex flex-col h-full transition-all duration-200 hover:transform hover:-translate-y-0.5 hover:shadow-card-hover"
    >
      <div className="w-full pt-[56.25%] relative bg-gray-200 overflow-hidden"> {/* 16:9 Aspect Ratio */}
        <img src={imageUrl} alt={title} className="absolute top-0 left-0 w-full h-full object-cover" />
      </div>
      <div className="p-5 flex flex-col flex-grow">
        <h3 className="text-lg font-semibold text-brand-gray-text mb-2 leading-tight">{title}</h3>
        <p className="text-sm text-brand-gray-textMedium leading-relaxed mb-3 flex-grow">{description}</p>
